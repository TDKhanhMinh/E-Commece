      ...
      <View className="flex-row items-center justify-between">
        ...
        <View className="flex-row items-center gap-3">
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
            className={`flex-row items-center px-3.5 py-2 rounded-2xl border ${isOnline
              ? 'bg-emerald-50 border-emerald-100 shadow-sm'
              : 'bg-slate-50 border-slate-100'
              }`}
          >
            <View
              className={`w-2 h-2 rounded-full mr-2 ${isOnline ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'
                }`}
            />
            <Text
              className={`text-[11px] font-black tracking-widest uppercase ${isOnline ? 'text-emerald-700' : 'text-slate-500'
                }`}
            >
          ...
          <View className="flex-row items-center ml-1">
            <TouchableOpacity
              ...
              activeOpacity={0.7}
              className="w-14 h-14 rounded-full bg-slate-50 border border-slate-100 items-center justify-center z-10"
            >
